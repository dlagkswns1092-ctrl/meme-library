    ...
    const syncUser = async () => {
      try {
        ...
        await fetch("http://localhost:8080/api/users/sync", {
          ...
          body: JSON.stringify({ email: user.email, nickname: nickname }),
        });
      } catch (error) {
        ...
        <main className="loginPageFigma">
          <div className="loginCanvas">
            ...
            <section className="loginBox">
              ...
              >
                <input type="text" placeholder="이메일" className="loginInputFigma" autoComplete="username" />
                <input type="password" placeholder="비밀번호" className="loginInputFigma" autoComplete="current-password" />
                <div className="loginOptionsRow">
                  <label className="rememberRow">
                    <input type="checkbox" />
                    <span>로그인 상태 유지</span>
                  </label>
                  <button type="button" className="forgotBtn">비밀번호 찾기</button>
                </div>
